...
if [ ! $(cget root-token) ]; then
  ...
  curl -X PUT -d "$(sed "1q;d" /tmp/vault.init  | awk -F': ' '{print $2}')" http://127.0.0.1:8500/v1/kv/service/vault/unseal-key-1
  echo "Store unseal key 2 in consul"
  curl -X PUT -d "$(sed "3q;d" /tmp/vault.init  | awk -F': ' '{print $2}')" http://127.0.0.1:8500/v1/kv/service/vault/unseal-key-2
  echo "Store unseal key 3 in consul"
  curl -X PUT -d "$(sed "5q;d" /tmp/vault.init  | awk -F': ' '{print $2}')" http://127.0.0.1:8500/v1/kv/service/vault/unseal-key-3
  echo "Store unseal key 4 in consul"
  curl -X PUT -d "$(sed "7q;d" /tmp/vault.init  | awk -F': ' '{print $2}')" http://127.0.0.1:8500/v1/kv/service/vault/unseal-key-4
  echo "Store unseal key 5 in consul"
  curl -X PUT -d "$(sed "9q;d" /tmp/vault.init  | awk -F': ' '{print $2}')" http://127.0.0.1:8500/v1/kv/service/vault/unseal-key-5
  echo "Store root token in consul"
  curl -X PUT -d "$(sed "11q;d" /tmp/vault.init  | awk -F': ' '{print $2}')" http://127.0.0.1:8500/v1/kv/service/vault/root-token
...
vault write secret/hashiapp-demo password=H@rdT0Gu3ss
